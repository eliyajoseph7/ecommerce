                ...
                <div class="w-full pt-3">
                    <div class="flex justify-end pb-2">
                        <a href="{{ route('discount_form') }}"
                            ...
                            <span class="">{{ __('Add Discount') }}</span>
                        </a>
                                            ...
                                            @forelse ($data as $dt)
                                                <tr wire:key="{{ $dt->id }}"
                                                    ...
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->percentage }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->start_date?->format('M d, Y') }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->end_date?->format('M d, Y') }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->created_at->format('M d, Y') }}</td>
                                                    <td class="px-4 py-3 flex items-center justify-end space-x-1">
                                                        <a title="Update"
                                                            href="{{ route('discount_form_edit', $dt->id) }}"
            ...
            Livewire.on('confirm_delete', (id) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('delete_discount', {
                            'id': id
                        });
                    }
                });
