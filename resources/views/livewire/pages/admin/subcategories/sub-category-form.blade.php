    ...
    <div class="relative">
        <x-slot name="header">
            @include('includes.breadcrumb', [
                'main' => 'Sub Categories',
                'menu' => 'Add Sub Category',
            ])
        </x-slot>
        <div class="absolute -top-10 right-2 z-40">
            <a href="{{ route('sub_category_list') }}" class="text-red-500 hover:text-red-700" title="Close">
                        ...
                        <div class="col-span-4 mb-8 p-6 border rounded-lg bg-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 md:gap-3">
                                <div class="mb-4">
                                    ...
                                    <select wire:model.live="sub_categories.{{ $index }}.main_category_id"
                                    ...
                                    @error("sub_categories.$index.main_category_id")
                                        <span class="text-red-500 text-sm">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700">Category</label>
                                    <select wire:model.live="sub_categories.{{ $index }}.category_id"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        <option value="">Select Category</option>
                                        @foreach (isset($categories[$index]) ? $categories[$index] : [] as $cat)
                                            <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                                        @endforeach
                                    </select>
                                    @error("sub_categories.$index.category_id")
                                        <span class="text-red-500 text-sm">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700">Sub Category Name <span
                                            class="text-red-500 text-sm">*</span></label>
                                    <input type="text" wire:model.lazy="sub_categories.{{ $index }}.name"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                            ...
                            <div class="mb-4 hidden">
                                ...
                                <input type="text" wire:model="sub_categories.{{ $index }}.slug"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                                @error("sub_categories.$index.slug")
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Status <span
                                        class="text-red-500 text-sm">*</span></label>
                                        <select wire:model.live="sub_categories.{{ $index }}.status"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        <option value="active">Active</option>
                                            <option value="in active">In Active</option>
                                    </select>
                                @error("sub_categories.$index.status")
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
